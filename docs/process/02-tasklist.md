...
| Внедрение SQLite | 📋 Запланировано | P0 | `[заполняется при старте]` |
...
### 5. Эпик: SQLite для локальной разработки
**Цель:** Внедрить SQLite в качестве полноценной локальной базы данных для разработки и тестирования. Это позволит безопасно выполнять сквозные CRUD-операции, не затрагивая рабочую (production) базу данных, и значительно упростит онбординг новых разработчиков.

**Статус:** 📋 Запланировано.

**Ключевые артефакты (будут созданы или изменены):**
- `package.json` (добавление зависимостей `sqlite3` и скрипта `db:init:sqlite`)
- `src/lib/env.ts` (реализация функции `useSqliteDev`)
- `src/lib/db.ts` (создание универсального адаптера для БД)
- `db/schema-portable.sql` (создание SQL-схемы, совместимой с SQLite)
- `scripts/init-sqlite.ts` (создание скрипта для инициализации и наполнения локальной БД)
- `src/lib/__tests__/db.test.ts` (создание интеграционных тестов для адаптера)
- `.gitignore` (добавление `*.sqlite` в список исключений)
- `docs/recipes/sqlite-dev.md` (создание подробной документации по настройке и использованию)
