...
export async function getProducts(): Promise<Product[]> {
  if (isLocal()) {
    return mockProducts; // из src/lib/mock-data.ts// ... (другой код)
...
## 🔧 Локальная разработка и База Данных

Для локальной разработки используется **SQLite**. Это позволяет вести полноценную разработку и тестирование сквозных сценариев (включая CRUD-операции) без подключения к облачной БД.

- **Сброс локальной БД:** `npm run db:reset-sqlite` — полностью удаляет файл `dev.sqlite`.
- **Инициализация локальной БД:** `npm run db:init:sqlite` — создает `dev.sqlite` и наполняет его схемой и данными.
- **Важно:** Файлы базы данных (`dev.sqlite`, `dev.sqlite-shm`, `dev.sqlite-wal`) никогда не должны попадать в коммит. Убедитесь, что они добавлены в `.gitignore`.
