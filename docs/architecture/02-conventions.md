...
- **Инициализация/сброс локальной БД:** `npm run db:init:sqlite`
- **Важно:** Файл базы данных (`*.sqlite`, `*.sqlite-journal`) никогда не должен попадать в коммит. Убедитесь, что он добавлен в `.gitignore`.
