...
> **Примечание о локальной разработке:**
> Этот документ описывает основную ("боевую") схему данных для **PostgreSQL**. Для локальной разработки используется адаптированная версия этой схемы для **SQLite**, которая находится в файле `db/schema-portable.sql`.
...
### 3.1 Среда для локальной разработки (Local Development Environment)

**Статус:** ✅ **Актуально**

Ранее, на начальном этапе проекта, для разработки UI-компонентов использовался подход с **`mock-данными`** (статичными объектами в коде).

Этот метод был признан **устаревшим (deprecated)** и заменен по следующим ключевым причинам:
1.  **Отсутствие CRUD:** Невозможность полноценно тестировать создание, обновление и удаление данных.
2.  **Разрыв E2E-сценариев:** Невозможность проверить сквозные пользовательские пути (например, 'добавление в корзину -> оформление заказа').
3.  **Рассинхронизация с реальностью:** Mock-данные быстро устаревали и переставали соответствовать реальной схеме БД.

**Текущее решение:** Для локальной разработки и тестирования проект использует **SQLite**. Это полноценная локальная база данных, которая полностью симулирует рабочую среду и позволяет проводить надежное E2E-тестирование.

Подробная инструкция по настройке и использованию находится в документе **`docs/recipes/sqlite-dev.md`**.
