...
async function create(productData: any): Promise<Product> {
    if (isLocal()) {
        productsServiceLogger.warn("Running in local mode. Mocking product creation.");
...
async function update(id: string, productData: any): Promise<Product> {
    return runLocalOrDb(
        () => {
