...
async function getAll(filters: ProductFilter = {}): Promise<Product[]> {
    ...
    try {
        ...
        if (searchQuery) {
            ...
            // Замена ILIKE на LIKE для совместимости с SQLite
...
async function getById(id: string): Promise<Product | null> {
    ...
    try {
        ...
        const validatedRow = validateDbRows(rows, DbProductSchema, "product");
        return mapDbProductToProduct(validatedRow[0]); 
    } catch (error) {
...
async function create(productData: ProductCreateInput): Promise<{ success: boolean; message: string; product?: Product }> {
    ...
    try {
        ...
        const validatedRow = validateDbRows(rows, DbProductSchema, "product");
        const product = mapDbProductToProduct(validatedRow[0]);
        ...
        if (isDbError(error) && error.code === "23505") { // PostgreSQL specific code
        ...
        // General error for SQLite and others
...
async function update(id: string, productData: Partial<ProductUpdateInput>): Promise<{ success: boolean; message: string; product?: Product }> {
    ...
    try {
        ...
        if (error.message.includes('UNIQUE constraint failed') && error.message.includes('title')) {
            return { success: false, message: "Товар с таким названием уже существует." };
        }
        return { success: false, message: "Произошла непредвиденная ошибка в базе данных." };
    }
...
async function remove(id: string): Promise<{ success: boolean; message: string }> {
    ...
    try {
        // Замена NOW() на параметр для совместимости
        const timestamp = new Date().toISOString();
        const { rowCount } = await query(`UPDATE products SET deleted_at = $1 WHERE id = $2 AND deleted_at IS NULL`, [timestamp, id]);
